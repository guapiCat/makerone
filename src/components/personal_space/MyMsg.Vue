<template>
    <div class="hello compon_center">

        <!--content start-->
        <div class="am-g">
            <div class="am-u-sm-7 am-u-sm-centered" style="width:1200px;padding-top: 50px;padding-bottom: 70px">
                <span style="float: left"><img src="../../../static/img/icon_address.png"></span>

                <div style="float: left;padding-top: 6px;margin-left: 3px;">
                    <span style="color: #999999;">益谷创客平台-个人空间-我的消息</span>
                </div>
            </div>

            <div class="am-u-sm-7 am-u-sm-centered" style="width:1200px;padding-top: 10px;">
                <div class="am-g" style="background-color: #FFCA57;padding-top: 1%;">
                    <div>
                        <ul class="am-nav am-nav-pills am-g" id="select_page">
                            <li style="margin-left: 17px" id="courseCollection" class="select_a gxMsg"><a href="#" id="courseCollection_a">
                                <div>新消息</div>
                            </a></li>
                            <li id="workCollection" class="select_a"><a href="#" id="workCollection_a">
                                <div>历史消息</div>
                            </a></li>
                        </ul>
                    </div>
                </div>
                <div class="content am-g"
                     style="height:90%;border:1px solid #f0f0f0;margin-bottom: 150px;padding-bottom: 1%;">

                    <!--<iframe id="iframe" src="message-history.html" scrolling="no" frameborder="0" style="width: 100%;"-->
                            <!--onload="iframeLoad()"></iframe>-->
                    <div>
                        <ul class="am-pagination am-pagination-right" style="margin-right: 2%;">
                            <li class="am-disabled"><a href="#">&laquo;</a></li>
                            <li class="am-active"><a href="#">1</a></li>
                            <li><a href="#">2</a></li>
                            <li><a href="#">3</a></li>
                            <li><a href="#">4</a></li>
                            <li><a href="#">5</a></li>
                            <li><a href="#">&raquo;</a></li>
                        </ul>
                    </div>
                </div>
            </div>
        </div>
        <!--foot-->
    </div>
</template>

<script type="es6">
    export default {
        name: 'hello',
        data () {
            return {
                msg: 'Welcome to Your Vue.js App'
            }
        }
    }


</script>

<style scoped>
    /*改动的样式-yzh*/
    .gxMsg{
        background: #FFCA57;
    }
    /*改动样式结束*/
    .topbar {
        background-color: #21252E;
    }

    i {
        font-style: normal;
    }

    .header .logo img {
        height: 50px !important;

    }

    .am-nav > li > a:hover, .am-nav > li > a:focus {
        outline: none;
    }

    .am-nav-justify > li {
        width: 150px;
    }

    .am-slider .am-slides img {
        height: 400px;
    }

    .section--header span {
        color: #3FA6E7;
    }

    .section--header img {
        height: 50px;
        margin-left: -145px
    }

    .section--header span {
        display: block;
        margin-left: 60px;
    }

    .am-thumbnails > li {
        padding: 0 3rem 1rem .5rem;
    }

    .login_table tr {
        width: 100%;
        display: block;
        margin: 10px;
    }

    .login_table tr td {
        text-align: center;
        width: 915px;
    }

    .login_table a {
        font-size: 10px;
    }

    .am-form input[type="password"] {
        padding: 0.9em;
    }

    table tr {
        display: block;
        margin-bottom: 30px;
    }

    td {
        padding-right: 55px;
    }

    #select_page a {
        font-size: 20px;
        color: #000;
        cursor: pointer;
    }

    #select_page {
        background-color: #ffffff;
        height: 4%;
        border-left: 1px solid #f0f0f0;
        border-right: 1px solid #f0f0f0;
    }

    #select_page img {
        margin-bottom: 9px;
        margin-right: 5px;
        width: 20%;
        height: 20%
    }

    #select_page div {
        float: left;
        width: 140px;
        text-align: center;
    }

    .am-nav > li {
        height: 55px;
    }

    #type_bar span {
        color: #a5a5a4;
    }

    #type_bar a {
        color: #a5a5a4;
    }

    #type_bar a:hover {
        color: #22a2d4;
    }

    #status_bar a {
        color: #a5a5a4;
    }

    #status_bar a:hover {
        color: #22a2d4;
    }

    .left-img img {
        height: 260px;
        width: 500px;
        margin-top: 10px;
        margin-left: 5%;
        border: 1px solid #000000;
    }

    /*导航栏*/
    #x_bg {
        background: url(../../../static/img/img_mynav.png) center center !important;
    }

    #x_nav {
        margin-left: 12% !important;
    }

    #x_nav ul li:nth-child(6) {
        border-right: 0;
        padding-right: 0;
    }

    .nav_img {
        float: left;
        width: 64px;
    }

    .nav_img img {
        width: 160px;
        margin-top: 10px;
    }

    .topbar {
        height: 50px;
    }

    .nav-search {
        float: left;
        width: 260px;
        border: solid 1px #fff;
        border-radius: 25px;
        margin-left: 41%;
        margin-top: 6px;
        height: 34px;
        line-height: 28px;
    }

    .nav-search img {
        margin-left: 19px;
        height: 22px;
    }

    .nav-search input {
        margin-left: 11px;
        background: #21252E;
        border: #21252E;
        color: #FFFFFF;
    }

    .p_head_photo {
        height: 32px;
        float: right;
        margin-left: 30px;
        color: #000000;
    }

    #x_nav ul li {
        border-right: dotted 1.5px #fff;
        padding-right: 37px;
    }

    /*导航栏结束*/
</style>