<template>
    <div>
        <div class="am-g">
            <div class="cousera" v-for="item in uncompleteWork"
                ...
                <div class="am-g" style="margin-left: 0px">
                    <div class="am-u-sm-6" style="padding-top: 1%;padding-left: 2%;">
                        <img class="cous-img" :src="fileURL+item.worksCoverImage" style="border: 1px solid #000000"
                             width="100%;"/>
                    </div>
                ...
                <div class="am-g">
                    <div class="am-u-sm-6 am-u-sm-offset-6" style="margin-top: 2%;">
                        <a class="am-btn am-btn-primary" style="border-radius: 7px;width: 130px;"
                           @click="delect(item.id)">移除作品</a>
                        <router-link :to="{name: 'unuploadAlltype',params:{workId:item.id}}"
                                     class="am-btn am-btn-primary"
                                     style="border-radius: 7px;width: 130px;float: right;margin-right: 17%;">编辑作品
                        </router-link>
                    </div>
                </div>
            </div>
            <ul v-if="uncompleteWork.length>0" class="am-pagination" style="margin-left: 700px;">
            ...
            <div v-if="uncompleteWork.length==0" style="padding-left: 50px;">请去完成一个作品吧(*^▽^*)</div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            getAllMsg:function(pageNum,pageSize){
                AXIOS.get('user/getUnfinishedWorks', {
                    params: {
                        "pageNum": pageNum,
                        "pageSize": pageSize
                    }
                }).then(response=> {
                    this.uncompleteWork = response.data.list;
                    //准备分页数据开始start
